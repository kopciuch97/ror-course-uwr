  ...
  describe 'GET #index' do
    ...
    context 'with a few normal events' do
      ...
      it do
        ...
        expect(assigns(:events)).to eq([event, event1, event2, event3, event4, event5])
        expect(assigns(:events_promoted)).to eq([])
      end
    end
    ...
    context 'with a few normal events and a few promoted events' do
      ...
      let!(:event1) { create :event }
      let!(:event2) { create :event }
      let!(:event3) { create :promoted_event }
      let!(:event4) { create :promoted_event }
      let!(:event5) { create :promoted_event }

      it do
        subject
        expect(assigns(:events).length).to eq(3)
        expect(assigns(:events)).to eq([event, event1, event2])
        expect(assigns(:events_promoted).length).to eq(3)
        expect(assigns(:events_promoted)).to eq([event3, event4, event5])
      end
    end
  end
